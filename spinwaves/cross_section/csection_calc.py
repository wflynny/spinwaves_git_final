...
#                                         EIG_SYM, WT_SYM, temperature, FF_SYM, eief, efixed)*sp.sin(THETA_SYM)*RAD_SYM**2
#    val_func = sp.lambdify((THETA_SYM, PHI_SYM, RAD_SYM, TAU_SYM, EIG_SYM, WT_SYM, FF_SYM), 
#                           val_expr, modules = "numpy")
#    cs_vals = val_func(thetas[:,nax,nax,nax], phis[:,nax,nax,nax], rad_list[:,nax,nax,nax], tau_list[nax,nax,:,nax], 
#                       eig_list[:,:,nax,nax], wt_list[nax,nax,nax,:], ff_list[:,nax,nax,nax]).sum(axis=2).sum(axis=1)
...
#    res_array = func(rad_list[:,nax,nax,nax],wt_list[nax,nax,nax,:],eig_list[:,:,nax,nax],
#                     tau_list[nax,nax,:,nax],ff_list[:,nax,nax,nax]).sum(axis=2).sum(axis=1) # same as .sum(axis=1).sum(axis=1)
